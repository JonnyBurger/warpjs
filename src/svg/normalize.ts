...
export function shapesToPaths(element: SVGElement) {
  const shapeMap = {
    ...
    polyline(shapeElement: SVGPolylineElement) {
      return pathShape.polyline(...shapeElement.points);
    },

    polygon(shapeElement: SVGPolygonElement) {
      return pathShape.polygon(...shapeElement.points);
    },

    rect(shapeElement: SVGRectElement) {
    ...
    ellipse(shapeElement: SVGEllipseElement) {
    ...
    circle(shapeElement: SVGCircleElement) {
  ...
  const shapeElements = element.querySelectorAll(
    Object.keys(shapeMap).join(",")
  );
  ...
  for (let shapeElement of shapeElements) {
    ...
    if (shapeName in shapeMap) {
      ...
      for (let attribute of Array.from(shapeElement.attributes)) {
        ...
        if (
          !/^(x|y|x1|y1|x2|y2|width|height|r|rx|ry|cx|cy|points|d)$/.test(name)
        ) {
          // @ts-expect-error
...
export function preparePaths(element: SVGElement, curveType = "q") {
