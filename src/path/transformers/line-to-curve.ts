import { isDrawingSegment, Segment, SegmentType } from "../utils";
...
export default function lineToCurveGenerator(curveType = "q") {
  ...
  return function lineToCurve(segment: Segment) {
    ...
    if (segment.type === "l") {
      ...
      segment.type = curveType as SegmentType;
    ...
    prevX =
      "x" in segment ? (segment.relative ? prevX : 0) + segment.x! : prevX;
    prevY =
      "y" in segment ? (segment.relative ? prevY : 0) + segment.y! : prevY;
